...
def tf_accuracy(predict_data, test_data):
    X = tf.constant(predict_data[:10,...], dtype=tf.float32, shape=test_data.shape)
    Y = tf.constant(test_data[:10,...], dtype=tf.float32, shape=test_data.shape)
...
def np_f1score(predict_data, test_data):
    ...
    print(np.argmax(predict_data, axis=1).shape)
    predict = np.argmax(predict_data, axis=1).reshape(30 * 565 * 565)
    test = np.argmax(test_data, axis=1).reshape(30 * 565 * 565)
...
if __name__ == "__main__":
    predict_data = np.load('cache/train_predict2_class_4_dev2_soft_relu.npy')
    test_data = prepare_image.load_images(data_type="train", image_type="label", classification=4,
                                            dataset="small")
