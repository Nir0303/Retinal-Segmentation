...
class RetinaSoftMaxModel(BaseModel):
    def __init__(self, classification=3, dataset="big", reload=False, activation='relu', cache=True):
        super(RetinaSoftMaxModel, self).__init__(classification, dataset, reload, activation, cache)
    ...
    def set_weights(self):
        if self.cache and os.path.exists("cache/keras_crop_model_weights_4class_reg_soft_relu.h5"):
            print("yes")
            self.model.load_weights("cache/keras_crop_model_weights_4class_reg_soft_relu.h5")
    ...
    def run(self):
        ...
        self.model.save_weights(os.path.join('cache', 
                                             'keras_crop_model_weights_4class_reg_soft_{}.h5'.format(self.activation)))
    
    def predict(self):
        test_predict = self.model.predict(self.test_images, batch_size=10)
        print(test_predict[0])
        print(test_predict.shape)
        np.save('cache/test_predict2_class_4_soft_{}.npy'.format(self.activation), test_predict)


if __name__ == '__main__':
    args = parse_args()
    rm = RetinaSoftMaxModel(classification=args.classification, dataset=args.dataset,
